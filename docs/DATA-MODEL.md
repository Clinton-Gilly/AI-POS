...
Indexes: `by_business`, `by_user`, `by_business_and_user` (uniqueness enforced in the mutation),
...
Indexes: `by_business`, `by_business_and_sku` (uniqueness enforced in the mutation),
`by_business_and_barcode` (uniqueness enforced in the mutation), `by_business_and_category`,
...
Indexes: `by_business_and_location_and_product` (uniqueness enforced in the mutation),
...
Indexes: `by_business_and_completed_at`, `by_business_and_number` (uniqueness enforced in the mutation),
`by_business_and_client_request_id` (uniqueness enforced in the mutation), `by_business_and_customer`,
...
`by_business_and_idempotency_key` (uniqueness enforced in the mutation), `by_business_and_status`
...
Indexes: `by_business`, `by_business_and_phone` (uniqueness enforced in the mutation),
...
> **On "unique" below.** Convex has no unique indexes. Every uniqueness
> guarantee in this document is enforced by a check inside the mutation that
> writes, which is safe because Convex mutations run at serializable isolation
> under optimistic concurrency control — a conflicting concurrent write aborts
> and retries into the already-handled branch. The indexes named make those
> checks cheap; they do not enforce the constraint. See the correction at the
> top of ADR-0003 and `convex/lib/uniqueness.ts`.
