...
This table *is* the authorization anchor. Server-side context resolution reads
...
One document per product per location — deliberately *not* one aggregate
...
somewhere to propose *into*, and because receiving stock is the natural source
...
A sale has *many* payments — split tender (part cash, part M-Pesa) is normal in
...
Indexes: **`by_provider_and_external_id` (unique — this index *is* the
